...
Do not expose tenant role summaries in the shared platform chrome for normal users. Sidebar footers, top-bar identity blocks, and profile dropdown headers should avoid showing role strings like `Owner`, `Reviewer`, `Tenant Admin`, or similar unless the session is a real platform admin session. General users only need identity and navigation; tenant-level permissions, internal developer access, or elevated workspace roles are not enough to show role labels in the shared chrome.
